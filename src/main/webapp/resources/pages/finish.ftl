<!DOCTYPE html>
<html>
<head lang="en">
    <meta charset="UTF-8">
    <title></title>
    <script src="/js/jquery-2.1.1.min.js" type="text/javascript"></script>
    <link rel="stylesheet" type="text/css" href="/css/finish.css">
</head>
<body>
<div class="finish-img">
    <img src="/images/icon_success.png">
</div>
<div class="finish-title">
    报名成功
</div>
<div class="finish-info">
    <div class="line"></div>
    <div class="info-box">
        <table style="width:100%">
            <tr>
                <td class="info-img"><img class="img-title" src="/images/detail-title.jpg"></td>
                <td class="info-desc">
                    <div class="info-name">2016行远帆船夏令营</div>
                    <div class="info-time">时间：2016/08/24-2016/08/28</div>
                    <div class="info-price">￥2800</div>
                </td>
            </tr>
        </table>
    </div>
    <div class="line"></div>
    <div class="children-info">
        <table style="width:100%">
            <tr>
                <td class="photo-td"><img class="photo-img" src="/images/photo.jpg"></td>
                <td class="children-td">
                    <div class="children-name">张筱雨（Daisy Zhang）</div>
                    <div class="children-age">女 13岁</div>
                    <div class="children-school">静安实验小学</div>
                </td>
            </tr>
        </table>
    </div>
</div>
<div class="order-box">
    <div class="line"></div>
    <div class="order-price">￥0.01+￥0.00运费</div>
    <div class="real-price">实付0.01</div>
    <div class="line"></div>
    <div class="order-id">订单号：E467890854234567898765</div>
    <div class="order-time">2016/04/09 16:10:00</div>
    <div class="order-time">完成付款</div>
</div>
</body>
</html>